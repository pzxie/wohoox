        ...
        return (
          <div>
            {[...map.values()].map(value => (
              <span key={Math.random()} role="string">
                {value}
              </span>
            ))}
    ...
    it('number -> string', () => {
      ...
      function Child() {
        ...
        return (
          <div>
            {[...map.values()].map(value => (
              <span key={Math.random()} role="number">
                {value}
              </span>
            ))}
    ...
    function renderItem(current) {
      return typeof current === 'bigint'
        ...
        : typeof current === 'object' &&
          current !== null &&
          (current.toString() === '[object Set]' ||
            current.toString() === '[object Map]')
        ? [...current].toString()
    ...
    it('edit all type', () => {
      ...
      function Child() {
        ...
        return (
          <div>
            <div role="outer">
              {<span role="item">{renderItem(current)}</span>}
            </div>
      ...
      expect(screen.getByRole('item').innerHTML).toBe(
        renderItem(allItems[0][1]),
      )
      ...
      for (let index = 0; index < allItems.length; index++) {
        ...
        switch (typeof originItem) {
          ...
          case 'object':
            if (originItem === null)
              newItemRenderText = JSON.stringify('null_changed')
              ...
              newItemRenderText = JSON.stringify({
                ...originItem,
                changed: 'changed',
              })
