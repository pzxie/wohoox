...
describe('set single component', () => {
    ...
    it('delete item', () => {
      ...
      for (let index = 0; index < allLength; index++) {
        ...
        if (allLength - index - 1 === 0) {
          ...
          expect(screen.getAllByRole('item').length).toBe(allLength - index - 1)
      }
    })
  })
  ...
  describe('edit item', () => {
    ...
    function renderItem(current) {
      ...
      if (current.toString() === '[object WeakMap]')
        return current.get(objectKey)
...
describe('set multi component', () => {
  ...
  describe('edit item', () => {
    ...
    function renderItem(current) {
      if (current.toString() === '[object Set]') return [...current].toString()
      if (current.toString() === '[object WeakSet]')
        return current.has(newWeakMapKey) + ''
      if (current.toString() === '[object Map]')
        return [...current.values()].toString()
      if (current.toString() === '[object WeakMap]')
        return current.get(newWeakMapKey)
      if (Array.isArray(current)) return current.toString()
      if (typeof current === 'object') return JSON.stringify(current)
    }
...
describe('weakSet multi component', () => {
  ...
  describe('delete item', () => {
    it('delete item', () => {
      ...
      function Parent() {
        let hasObject: Map<any, any> = useStore(state =>
          state.set.has(objectKey),
        )
